...
Original file is located at
    https://colab.research.google.com/drive/1wbHlLLfWGI4B2ysTUKc5GZfTmdkFc5n5
"""
...
csv_file = pd.read_csv('set_a.csv',usecols=['fname','label'])
paths = np.array(csv_file.loc[:,"fname"])
labels = np.array(csv_file.loc[:,"label"])

print('COMPROBACION', np.any("normal" in labels))
...
for i in range(len(paths)):
    ...
    if os.path.exists(paths[i]):
        ...
        duration = librosa.get_duration(filename=paths[i])
...
#                 initial_offset = (duration % slice_size)/2
            ...
            for j in range(iterations):
                #print(i)
                print(labels[i])
                offset = initial_offset + j*(slice_size-1)
                #print(labels[i])
                if (labels[i] == "normal"):
                    dataset.append({
                        "filename": paths[i],
                ...
                elif labels[i] != "normal" and labels[i] != "NaN":
                    dataset.append({
                        "filename": paths[i],
                        "label": "abnormal",
                        "offset": offset
